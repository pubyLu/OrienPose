...
ckpt_path = "./cache/dino_weight.pt"
print(ckpt_path)
...
if __name__ =="__main__":
    ...
    #     # name = 'guantou'
    #     # 不能开梯子
    #     root_dir = r"D:\A_ProductionBase\AAAI\LYT\orientImage"
    #     for seq in os.listdir(root_dir):
    #         print(f"============{seq}=============")
    #         seq_dir = os.path.join(root_dir, seq)
    #         pre_save_path = fr"D:\A_ProductionBase\AAAI\LYT\orientImage\result\{seq}"
    #         if not os.path.exists(pre_save_path):
    #             os.makedirs(pre_save_path)
    #         for img_file in os.listdir(seq_dir):
    #             img_path = os.path.join(seq_dir, img_file)
    #             img = cv2.imread(img_path)
    #             res_img, _, _, _, _= infer_func(img, do_rm_bkg=False, do_infer_aug=False)
    #             res_img = np.array(res_img)
    #             cv2.imwrite(os.path.join(pre_save_path, img_file), res_img)
        # for degree in [0, 30, 60, 90, 120, 150, 180]:
        #     img = cv2.imread(rf"D:\B-paperTestRelated\idea-ratationRelated\{name}_{degree}.{type}")
        #     res_img, _, _, _, _= infer_func(img, True, False)
        #     res_img = np.array(res_img)
            # cv2.imwrite(rf"D:\B-paperTestRelated\idea-ratationRelated\{name}_{degree}_axis.jpg", res_img)
            # cv2.imshow("res", res_img)
            # cv2.waitKey(0)
            # cv2.destroyAllWindows()

    # img = cv2.imread(r"D:\A_ProductionBase\AAAI\ZY\cat.png")
    # res_img, _, _, _, _ = infer_func(img, True, False)
    # res_img = np.array(res_img)
    # cv2.imwrite(rf"D:\A_ProductionBase\AAAI\ZY\cat_axis.png", res_img)
    seq_dir = r"D:\A_ProductionBase\AAAI\LYT\Nope_dataset\shapenet\orient"
    ...
    for img_file in os.listdir(seq_dir):
        ...
        res_img, _, _, _, _= infer_func(img, do_rm_bkg=False, do_infer_aug=False)
        res_img = np.array(res_img)
        cv2.imwrite(os.path.join(seq_dir, "orient_"+img_file), res_img)
        break